...
const NewPassword = () => {
    ...
    const GetRegister = () =>{
        const getdata = JSON.parse(localStorage.getItem('Register-User'));
        const getotp = JSON.parse(localStorage.getItem('Otp-User'));
        
        console.log('Register-Get++', getdata);
        console.log('Otp-Get++', getotp);

        setgetregister(getdata);
        setgetotp(getotp);
    ...
    const HandleChange = () => {
        const a= getregister.find((i)=> i.id === getotp.id);
        ...
        if(password === cpassword){ 
            ...
            console.log('Password-Update++', changepassword);

            const updateregister = getregister.map((i) => i.id === changepassword.id ? changepassword : i);
            console.log('Register-Password-Update++', updateregister);

            localStorage.setItem('Register-User', JSON.stringify(updateregister));
            ...
            seterror("New Password and Confirm Password are not Same Please Enter Same Password");
        }
                        ...
                        <div className="card">
                            <div className="card-body">
                                ...
                                <div className="app-brand justify-content-center">
                                    <Link to="/" className="app-brand-link gap-2 x">
                                ...
                                <p className="mb-4">Your new Password must be different from your old password.</p>
                                <form className="mb-3">
                                    {error && <div class="alert alert-danger text-danger fw-bold" role="alert">{error}</div>}
                                    <div className="mb-4">
                                    ...
                                    <div className="mb-4">
                                        ...
                                        <div className="input-group input-group-merge">
                                            <input type="password" className="form-control" placeholder="············" value={cpassword} onChange={(e)=> setcpassword(e.target.value)} />
                                            <span className="input-group-text cursor-pointer"><i className="bx bx-hide" /></span>
                                        </div>
                                    </div>
                                    <button type="button" className="btn btn-primary d-grid w-100" onClick={HandleChange}>Change Password</button>
                                </form>
                            </div>
                        </div>
                        {/* /Forgot Password */}
