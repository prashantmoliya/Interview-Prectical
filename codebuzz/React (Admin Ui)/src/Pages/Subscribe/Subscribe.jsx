...
const Subscribe = () => {
    ...
    const GetSubscribe = async () => {
        ...
        try {
            const res = await Axios.get(`/subscribe/list`, authHeader());
    ...
    //     try {
    //         const res = await Axios.delete(`/subscribe/delete/${deleteId}`, authHeader());
    ...
    //             setSubscribe((prev) => prev.filter((i) => i.id !== deleteId));
    //         }
    //         else {
    //             toast.error(res.data?.message);
    //         }
    ...
    const filterSubscribe = subscribe.filter((i) => {
        const searchstr = `${i.email} ${CreatedDate(i.createdAt)}`.toLowerCase();

        return searchstr.includes(search.toLowerCase());
    });

    const startIndex = (currentPage - 1) * perPage;
    const currentPageData = filterSubscribe.slice(startIndex, startIndex + perPage);
...

                                {
                                    error ? (
                                        ...
                                        <DataTable
                                            ...
                                            pagination
                                            paginationServer
                                            paginationTotalRows={filterSubscribe.length}
