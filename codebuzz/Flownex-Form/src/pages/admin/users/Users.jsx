...
import AddUser from "../../../components/modal/admin/users/add/AddUser";
import EditUser from "../../../components/modal/admin/users/edit/EditUser";
import ViewUser from "../../../components/modal/admin/users/view/ViewUser";
import Share from "../../../components/modal/share/Share";

import { useDispatch, useSelector } from "react-redux";
import { reqtoAdminDeleteUser, reqtoAdminGetUser } from "../../../redux-Toolkit/services/admin/AdminServices";
...
const Users = () => {
    ...
    const { usersList, loader, deleteLoader } = adminReducer;
    ...
    const handleDelete = async () => {
        const res = await dispatch(reqtoAdminDeleteUser(modalState.deleteId));
    ...
    const GetUserList = async () => {
        await dispatch(reqtoAdminGetUser());
    }
                ...
                <div className="d-flex align-items-center">
                    ...
                    <button type="button" className="btn btn-sm btn-neutral text-nowrap eye-icon"
                        onClick={() => {
                            ...
                            setModalState({ ...modalState, shareUser: row });
                        }}
                    >
    ...
    const filterUsers = usersList?.filter((i) => {
        const searchstr = `${i.name} ${i.email} ${i.mobileNumber} ${CreatedDate(i.createdAt)}`.toLowerCase();
    ...
    return (
        <>
                            ...
                            <div className="card-body table-responsive">
                                {
                                    error ? (
                                        ...
                                        <DataTableComponents
                                            ...
                                            onRowClicked={(row) => navigate(`/admin/users/${row.id}`)}
                                            pointerOnHover
                                        // highlightOnHover
                                        />
                                    )
            ...
            {/* Share-User Modal */}
            <Share show={modalShow.shareUser} handleClose={handleClose} users={modalState.shareUser} GetUserList={GetUserList} />
        </>
    )
}
